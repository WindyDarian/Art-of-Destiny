...
namespace Stages.Stage1Parts
{
    ...
    public class Stage1_Part4:StagePart
    {
        ...
        public override void Initialize()
        {
            ...
            Stage.AddGameMessage(@"杰诺：看来它们已经控制了这个地方，在支援舰队之前，必须先陪它们玩一下。", Color.LightGreen, 3);
        ...
        public override void Touch()
        {
            if (b1 == true && b2 == false )
            {
                if (Stage.AliveUnitsInUnitGroup(2).Count <= 0)
                {
                    ...
                    Stage.AddGameMessage(@"比尔：小心你的背后！", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：看来你需要帮忙，朋友，我是比尔.沃顿，第三舰队的前成员。", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：你正在使用的行星突击炮，是很好的武器，", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：它可以轻松刺穿敌人的能量护盾，但是对护甲的伤害并不理想。", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：如果你让它有足够长的时间冷却，再次射击时就会有短时间的急速状态。", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：秩序之眼的能量护盾都是为防御导弹的攻击而安装的，", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"比尔：如果你在用行星突击炮破掉了对手的护盾后再发射导弹的话……", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"杰诺：够了，你以为我是新手吗？ ", Color.LightGreen, 2);
                    Stage.AddGameMessage(@"比尔：随便。但是我认为我们现在必须把这里清理一下。", Color.CornflowerBlue, 2);
                    Stage.AddGameMessage(@"杰诺：总之，我是老鸟，这就够了。", Color.LightGreen, 2);
            ...
            if (t2!= null)
            {
                if ((t2.IsEnd || Stage.AliveUnitsInUnitGroup(2).Count <= 0) && (b4 == false && b3== true))
                {
                    ...
                    Stage.AddGameMessage(@"比尔：这些家伙是秩序之眼的精英！", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"杰诺：比废铁有用一点罢了。", Color.LightGreen, 2);
                    b4 = true;
                }
            }
        ...
        public override void Event_TimerRing(Timer timer)
        {
            if (timer == t1 && b3 == false)
            {
                ...
                Stage.AddGameMessage(@"比尔：朋友，看来我们被包围了。", Color.CornflowerBlue, 4);
                Stage.AddGameMessage(@"比尔：这些家伙带有导弹，时刻注意自己的能量护盾。", Color.CornflowerBlue, 4);
                Stage.AddGameMessage(@"杰诺：这就是本游侠大显身手的机会么？ ", Color.LightGreen, 3);
                b3 = true;
