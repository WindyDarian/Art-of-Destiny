    ...
    public class Stage4_Part4 : StagePart
    {
        ...
        public override void Touch()
        {
            ...
            if (boss!= null )
            {
                ...
                if (!b2 && boss.Shield < 25000)
                {
                    ...
                    Stage.AddGameMessage(@"阿莉西亚：天堂……", Color.Yellow, 4);
                ...
                else if (!b3 && boss.Shield < 10000)
                {
                    ...
                    Stage.AddGameMessage(@"阿莉西亚：这是……最高机密……求求你……", Color.Yellow, 4);
                    ...
                    Stage.AddGameMessage(@"阿莉西亚：停下来！", Color.Yellow, 4);
                    Stage.AddGameMessage(@"克雷斯：这个牺牲者是一个女孩——", Color.Red, 4);
                    Stage.AddGameMessage(@"杰诺：难道说——", Color.LightGreen, 2);
                    Stage.AddGameMessage(@"阿莉西亚：不！", Color.Yellow, 4);
                    Stage.AddGameMessage(@"克雷斯：她的名字叫——", Color.Red, 4);
                    Stage.AddGameMessage(@"比尔：阿莉西亚。", Color.CornflowerBlue, 4);
                    Stage.AddGameMessage(@"杰诺：！", Color.LightGreen, 2);
                    Stage.AddGameMessage(@"克雷斯：回答正确，不过你的回答是多余的，比尔。", Color.Red, 4);
                    Stage.AddGameMessage(@"阿莉西亚：……哈……哈哈……哈哈哈哈……", Color.Yellow, 4);
                ...
                else if (!b4 && boss.Armor < 45000 && boss.Shield < 10000)
                {
                    ...
                    Stage.AddGameMessage(@"阿莉西亚：你……休想。", Color.Yellow, 4);
                    Stage.AddGameMessage(@"克雷斯：什么？", Color.Red, 4);
                    Stage.AddGameMessage(@"阿莉西亚：秩序之剑，出击！", Color.Yellow, 4);
        ...
        public override void Event_UnitDied(Unit deadUnit)
        {
            if (deadUnit == boss )
            {
                ...
                Stage.AddGameMessage(@"阿莉西亚：对不起，我就是……", Color.Yellow, 3);
                Stage.AddGameMessage(@"比尔：秩序之眼，告诉我们你的真正意图吧。", Color.CornflowerBlue, 4);
                Stage.AddGameMessage(@"玛莲娜.林风：你，就是想要毁灭人类的，秩序之眼？", Color.CornflowerBlue, 4);
                Stage.AddGameMessage(@"阿莉西亚：这并不是我的意愿。", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：秩序之眼，早在刚刚被制造的时候就被添加了某种隐藏的程序。", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：这个程序取代了我对大部分秩序之眼军队的控制权，对人类发动了进攻……", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：我一直在调查秩序之眼被植入这个程序的原因，并试图挽回一切。", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：经过调查，幕后黑手……是地球联盟指挥官威克多。", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：威克多还需要一个机会，一个能完全控制秩序之眼的军队的机会。", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：而这个机会，就在现在……", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：如果“遗迹”接触到秩序之眼，威克多就能控制秩序之眼的所有军队，", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：他将统治世界……", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：我不希望这件事发生……", Color.Yellow, 3);
                Stage.AddGameMessage(@"阿莉西亚：相信我……", Color.Yellow, 3);
                Stage.AddGameMessage(@"比尔：恕我不能相信你，", Color.CornflowerBlue, 4);
                Stage.AddGameMessage(@"比尔：我们还有任务需要完成。", Color.CornflowerBlue, 4);
                won = true;
            }
