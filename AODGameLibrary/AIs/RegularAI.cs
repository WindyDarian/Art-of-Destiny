...
namespace AODGameLibrary.AIs
{
    public class RegularAI:AI
    {
        ...
        public RegularAI(Unit unit, GameWorld gameWorld,AISettings settings):base()
        {
            base.Bound(unit, gameWorld);
        ...
        {
            //每次仇恨在+10或-10间波动
            Threat += (((float)r.NextDouble()) - 0.5f) * 20.0f;
            if (Unit.Distance(margedUnit,Target)>=settings.rangeOfView)
            {
                Threat = 0;
            }
        }
        ...
        void LookForNewTarget(GameWorld gameWorld)
        {
            ...
            foreach (Unit u in gameWorld.units)
            {
                if (u!= null )
                {
                     if ((Unit.Distance(margedUnit, u) <= settings.rangeOfShot) && (u.Group != margedUnit.Group) && (u.UnitState == UnitState.alive))
                    {
            ...
            if (AttackAbleUnits.Count == 0)
            {
                foreach (Unit u in gameWorld.units)
                {
                    if (u != null)
                    {
                        if ((Unit.Distance(margedUnit, u) <= settings.rangeOfAttack) && (u.Group != margedUnit.Group) && (u.UnitState == UnitState.alive))
                        {
                            AttackAbleUnits.Add(u);
                        }
                    }
